...
* Ruby 3.2.2
* Ruby on Rails 7.0.5
  * `rails new . -T -d=mysql -c=bootstrap`
* Use MySQL
* Haml
* FontAwesome
...
Use [setup.rb](https://raw.githubusercontent.com/h-izumi/skelton70/mysql/setup.rb):

```shell
cd /path/to/app-parent
curl -L https://raw.githubusercontent.com/h-izumi/skelton70/mysql/setup.rb | APP_NAME="app-name" ruby
```

* Set `NO_COMMIT=true` to skip `git commit`.

```shell
curl -L https://raw.githubusercontent.com/h-izumi/skelton70/mysql/setup.rb | APP_NAME="app-name" NO_COMMIT=true ruby
```
